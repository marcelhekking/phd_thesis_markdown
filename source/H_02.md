...
- $z_k$ has maximum possible variance among all possible linear
  ...
  uncorrelated with $z_1$, $z_2$,\...$z_{k-1}$, for 2 ≤ *k* ≤ $p$
  \[4\].
...
multivariate data set. For a set of $N$ vectors $\bold x_i (i= 1,...,N)$  in a $p$-dimensional data set, the variance-covariance
matrix V is defined as:

$$V = \frac{\sum_{i = 1}^{N}{(\bold x_i - \bold m)(\bold x_i - \bold m)^{T}}}{N(N - 1)}$$
(2--2)

where $\bold m$ is the vector of the mean of the set $\bold x_i (i= 1,...,N)$ and the superscript $T$ indicates transposition
...
which transforms the original vectors $\bold m$ into vectors $\bold y$,
according to $\bold y = U \bold x$, such that the variance-covariance matrix $W = UVU^T$ of the transformed vectors $\bold y$ is a diagonal matrix. If $U$ is
constrained to be a unitary matrix, the component variances of the
transformed vectors $\bold y$ appear as eigenvalues ($$\lambda$$) in the analysis and
are found as the diagonal elements of $W$.

Since the eigenvalue transformation diagonalises the variance-covariance
matrix, the total variance in the set of original vectors $\bold x$ is
decomposed into $p$ orthogonal directions. Thus, for a set of
$p$-dimensional vectors $\bold x$ for which it is observed that most of the
...
$\bold y$ contain most of the useful information. The components $l +1$
...
normalised eigenvectors $\epsilon$, which are used to calculate the associated
principal component values from a combination of standardised original
acid-base values. Table 2 --3 shows the normalised eigenvectors $\epsilon$ of
...
as  $m \plusmn 2 s$, resulting in a reference range of -0.472 to -0.092. The
