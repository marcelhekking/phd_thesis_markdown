...
combination of $\ce{pH}$, PaCO­~2~ and a$\ce{[HCO3-]}$ is not valid.
...
log $\ce{PaCO2}$ and log a$\ce{[HCO3-]}$, since the variance in PC3
...
check the plausibility of a transformed $\ce{pH}$, $\ce{PaCO2}$ and $\ce{BE}$ combination
