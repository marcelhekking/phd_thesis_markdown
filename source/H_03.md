...
disorders \[10\]. The 'followers' of the in vivo approach (see Chapter 1) mainly use this type of chart. When in 1960 Astrup and Siggaard-Andersen developed the concept of the base excess (BE) and the accompanying classification scheme, they proposed a chart for the representation of pH, PaCO~2~ and BE values. The Siggaard-Andersen chart is a plot of logarithmic PaCO~2~ versus pH in which equal BE values are represented by straight isopleths running form north-west to south-east \[11, 12\].
...
$$T = \begin{bmatrix}\text{cos}\ \alpha & \text{sin}\ \alpha & 0 \\ - \text{sin}\ \alpha & \text{cos}\ \alpha & 0 \\0 & 0 & 1 \\\end{bmatrix}\text{*U}$$
(3--1)
...
_Figure 3--5. The proposed chart for pH, PaCO~2~ and BE as built from the data in the ELIbe data set. The classical acid-base classification from Table 1--2 are plotted in (dotted lines)._
